# src_1d_cnn_optimized/train_kfold.py
import os
import numpy as np
...
from .models import create_cnn1d_model
from .viz import ensure_dir, save_loss_plot, save_accuracy_plot, save_cm_plot, save_metrics_table_as_svg
...
def main():
    ...
    for train_idx, test_idx in skf.split(X, y_int):
        ...
        model = create_cnn1d_model(timesteps, n_features, cfg.n_classes, lr=cfg.lr)

        early_stopping = EarlyStopping(monitor="val_loss", patience=cfg.es_patience, restore_best_weights=True)
        reduce_lr = ReduceLROnPlateau(monitor="val_loss", factor=cfg.rlrop_factor, patience=cfg.rlrop_patience, min_lr=cfg.min_lr)
    ...
    metrics_df.to_csv(os.path.join(cfg.results_dir, "cnn1d_kfold_metrics.csv"), index=False)
    save_metrics_table_as_svg(metrics_df, filename=os.path.join(cfg.results_dir, "cnn1d_kfold_metrics.svg"))
