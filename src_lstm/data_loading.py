...
def load_class_dir(data_root: str, class_dir: str, label: int, n_points: int):
    """
    Loads all files from data_root/class_dir and returns (X_list, y_list).
    """
    X_list, y_list = [], []
...
def build_X_y(data_root: str, class_map: dict[str, int], n_points: int, n_classes: int):
    ...
    X = np.asarray(X_all, dtype=np.float32)              # (N, n_points, n_features)
    y = np.asarray(y_all, dtype=int).reshape(-1, 1)      # (N, 1)
